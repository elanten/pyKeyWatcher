...
from digitalkey.models import DigitalKeyContacts
...
def edit_by_id(request: WSGIHandler, key_id):
    digital_key = get_object_or_404(DigitalKey, pk=key_id)
    holders = digital_key.contragent.filter(digitalkeycontacts__type=DigitalKeyContacts.HOLDER).values('id', 'name')
    contacts = digital_key.contragent.filter(digitalkeycontacts__type=DigitalKeyContacts.CONTACT).values('id', 'name')
    ...
    if request.method == 'POST':
        ...
        if digital_key_form.is_valid():
            ...
            digital_key.contragent.clear()
            holders_ids = request.POST.getlist('holders', [])
            contacts_ids = request.POST.getlist('contacts', [])
            for _id in holders_ids:
                DigitalKeyContacts(digital_key=digital_key, contragent_id=_id, type='h').save()
            for _id in contacts_ids:
                DigitalKeyContacts(digital_key=digital_key, contragent_id=_id, type='c').save()
...
def remove_by_id(request, key_id):
    digital_key = get_object_or_404(DigitalKey, pk=key_id)
    digital_key.delete()
    return redirect(request, 'digital_key:all')
